...
stations <- read.csv('data_input/weather_stations.csv', sep = ',', stringsAsFactors = FALSE)
# str(stations)
...
for (i in 1:length(years)){
  for (j in 1:length(months)){
    temp_pollution[[increment]] <- read.csv(paste0('data_input/pollution_data/hourly_data_',years[i],'_',months[j],'.csv'), sep=',', stringsAsFactors = FALSE)
